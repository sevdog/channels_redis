...
async def _close_redis(connection):
    ...
    """
    if isinstance(connection, aioredis.RedisCluster):
        try:
...
def create_pool(host):
    ...
    cluster_mode = host.pop("cluster_mode", False)
    if cluster_mode:
        startup_nodes = host.pop("startup_nodes", None)
        if startup_nodes is not None:
            startup_nodes = [
                aioredis.cluster.ClusterNode(host=node[0], port=node[1])
...
def ensure_redis_client(pool):
    """
    Takes a pool generated by ``create_pool`` and returns a suited client.
    """
    # redis cluster has an under-the-hood pool management and can be considered
    if isinstance(pool, aioredis.RedisCluster):
        return pool
